...
import { Image, Text, Button } from '@crosswise/uikit'
...
export const FarmHeadCard = styled.div<{ isDarkTheme: boolean; expanded: boolean }>`
  /* padding: 40px 24px; */
...
export const FarmCardsLayout = styled(FlexLayout)`
  display: grid;
  grid-template-columns: repeat(auto-fit, min(420px));
  /* grid-gap: 50px; */
  row-gap: 50px;
  column-gap: 3px;
  /* justify-content: space-between; */
  @media screen and (max-width: 1440px) {
    grid-template-columns: repeat(auto-fit, min(345px));
  }
`
