...
import { SwapRow, StyledCard, Wrapper } from './styled'
...
export default function Swap({ history }: RouteComponentProps) {
  ...
  const fetchChartData = async () => {
    ...
    const filteredData = data.slice(-arrayCount).map((semidata) => {
      ...
      return { value: semidata.low.toFixed(5), time: fullDate, fullTime: time }
    })
    ...
    <StyledCard>
      <Wrapper id="swap-page" flexDirection="column">
        <Flex width="100%" mb="30px" justifyContent="space-between" alignItems="center">
          ...
          <Dropdown
            list={graphOpts}
            current={graphResolution}
            placement="bottom-end"
            onClickItem={setGraphResolution}
          />
        </Flex>
        {/* <Box height="60px">
          {priceHover && (
