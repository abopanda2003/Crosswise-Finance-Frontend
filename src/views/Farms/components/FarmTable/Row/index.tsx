...
const Row: React.FunctionComponent<RowPropsWithLoading> = (props) => {
  ...
  const { details, userDataReady, index, farm, multiplier, liquidity } = props
    ...
    <StyledTr index={index} onClick={toggleActionPanel}>
      ...
      <RowBody>
        <Flex justifyContent="space-between" alignItems="center">
          ...
          {/* <GradientText fontWeight="bold">{farm.label}</GradientText> */}
          <Text fontWeight="bold" fontSize="21px" color={isDark ? '#fff' : '#060514'}>
            {farm.label}
          </Text>
          <Multiplier {...multiplier} />
        </Flex>
        {renderOrder.map((key) => {
          ...
          switch (key) {
            ...
            case 'apr':
              return (
                <CellInner key={key} alignItems="flex-start">
        ...
        {/* TODO: Following CellInner must be changed into staked */}
        <CellInner>
          <CellLayout label="STAKED">{React.createElement(Liquidity, { ...liquidity })}</CellLayout>
        </CellInner>
        <OptionContainer>
          <ToggleWrapper>
