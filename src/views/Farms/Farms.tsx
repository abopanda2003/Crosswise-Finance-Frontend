...
const Farms: React.FC = () => {
  ...
  const StakedOnlyToggleButtonContainer: JSX.Element = (
    <Flex justifyContent="flex-end">
      <StakingToggle>
        <ToggleWrapper>
          <LabelNameText fontSize="12px" pr="15px">
            {t('Staked only')}
          </LabelNameText>
        ...
        <Flex justifyContent="space-between">
          <Flex alignItems="center">
            <StyledSvgButton onClick={() => handleToggle(ViewMode.CARD)}>
              <CustomText>{viewMode === ViewMode.CARD ? <IconGridFill /> : <IconGridOutlined />}</CustomText>
            </StyledSvgButton>

            <StyledSvgButton onClick={() => handleToggle(ViewMode.TABLE)}>
              <CustomText>{viewMode === ViewMode.TABLE ? <IconListFill /> : <IconListOutlined />}</CustomText>
            </StyledSvgButton>
  ...
  const renderContent = (): JSX.Element => {
    ...
    return (
      <FarmCardsLayout justifyContent={isMobile ? 'center' : 'space-between'}>
    ...
    <>
      <FarmHeader>
        <HeaderTopBar isMobile={isMobile}>
          <Heading scale="xl" color="text" mb="12px" style={{ fontSize: isMobile ? '64px' : '76px' }}>
            Farm
          </Heading>

          {/* <Text fontSize="21px">{t('Stake LP tokens to Earn CRSS')}</Text> */}
          <Text fontSize={isMobile ? '16px' : '26px'}>Stake LP tokens to Earn CRSS</Text>
      ...
      <FarmHeaderLayout>
        ...
        <FarmHeadCard isDarkTheme={isDark} expanded={isMobile || headCardExpanded}>
          <FarmHeadCardHeader onClick={handleClickFarmHeadCardHeader}>
            {/* <GearIcon /> */}
            <div />
            <FarmHeadCardTitle>
              <StatsIcon />
              <Text color={isDark ? '#fff' : '#060514'} fontSize={isMobile ? '16px' : '26px'} fontWeight={700}>
                Farm Stats
              </Text>
            </FarmHeadCardTitle>
            {/* <DropDownUpIcon /> */}
          ...
          <Flex justifyContent="space-around" alignItems="center" mt={19}>
            <Flex flexDirection="column" alignItems="center" justifyContent="space-between">
              <Flex alignItems="center" flexDirection="column">
                {isMobile && <PendingRewardIcon />}
                <Text color={isDark ? '#BFC8DA' : '#7A8596'} fontSize={isMobile ? '12px' : '16px'} fontWeight={600}>
                  Pending Rewards
                </Text>
              </Flex>
              <Text color={isDark ? '#fff' : '#060514'} fontWeight={700} fontSize={isMobile ? '16px' : '26px'}>
              ...
              <Flex justifyContent="center" alignItems="center">
                <FarmHeadCardEarningPanel>
                  <Flex flexDirection="column" justifyContent="space-between" alignItems="center">
                    <LabelNameText color={isDark ? '#04F8AD' : '#3F81EF'} fontSize="16px" pr="8px">
                      $CRSS Earned
                    </LabelNameText>
                    <Text color={isDark ? '#fff' : '#060514'} fontWeight={700} fontSize="26px" mr="24px">
                      {insertThousandSeparator(CrssTokenEarned?.toFixed(2))}
                    </Text>
                    <Text color={isDark ? '#BFC8DA' : '#7A8596'} fontSize="12px" mr="24px">
